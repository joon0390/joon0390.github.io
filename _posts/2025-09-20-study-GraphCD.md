...
전통적인 방법들은 평균이나 분산 같은 **저차원 통계량**의 변화를 봅니다(CUSUM, LRT 등). 하지만 데이터가 점점 **고차원**, **비정형**, **복잡한 구조**를 갖게 되면서 한계가 뚜렷해졌습니다. 평균/분산만으로는 “관계나 구조의 변화”를 포착하기 어렵기 때문입니다.

바로 이런 문제의식에서 Chen & Zhang (2012)은 **데이터를 그래프로 표현**하고, **그래프 위에서 변화점을 스캔**하는 비모수적 접근을 제안합니다. 핵심은 간단하지만 효과적입니다: *비슷한 것끼리 연결해 보고, 시점을 나눴을 때 두 그룹을 잇는 간선이 얼마나 남는지 세어보자.*
...
**5) 다중 변화점**  
- $\max_t Z_G(t)$로 하나를 검정/추정 → 구간을 분할 → 반복하는 **바이너리 세그멘테이션**이 실무에서 널리 쓰입니다.  
